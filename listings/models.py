...
class Reserved(models.Model):
    booking_info = models.ForeignKey(
        BookingInfo, on_delete=models.CASCADE,
        related_name='reserved'
    )
    hotel_room_id = models.ForeignKey(
        HotelRoom,
        blank=True,
        null=True,
        on_delete=models.CASCADE,
    ...
    def __str__(self):
        if self.hotel_room_id:
            return f'{self.booking_info} Room No {self.hotel_room_id} Book from  {self.check_in} to {self.check_out}'
        else:
            return f'{self.booking_info} Book from  {self.check_in} to {self.check_out}'
